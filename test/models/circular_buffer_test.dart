...
import 'package:js_dash/core/circular_buffer.dart';
  ...
  group('CircularBuffer', () {
    ...
    test('adds points until capacity', () {
      ...
      for (int i = 0; i < bufferSize; i++) {
        buffer.add(
          TimeSeriesPoint(now.add(Duration(seconds: i)), i.toDouble()),
        );
    ...
    test('removes oldest point when adding beyond capacity', () {
      ...
      buffer.add(
        TimeSeriesPoint(
          now.add(Duration(seconds: bufferSize)),
          bufferSize.toDouble(),
        ),
      );
    ...
    test('removeOldData removes points before cutoff', () {
      ...
      for (int i = 0; i < bufferSize; i++) {
        buffer.add(
          TimeSeriesPoint(now.add(Duration(minutes: i)), i.toDouble()),
        );
      }
    ...
    test('removeOldData handles all data being old', () {
      ...
      for (int i = 0; i < bufferSize; i++) {
        buffer.add(
          TimeSeriesPoint(now.subtract(Duration(hours: i + 1)), i.toDouble()),
        );
      }
    ...
    test('handles rapid continuous updates at capacity', () {
      ...
      for (int i = 0; i < totalUpdates; i++) {
        buffer.add(
          TimeSeriesPoint(now.add(Duration(milliseconds: i)), i.toDouble()),
        );
      }
      ...
      expect(
        points.first.value,
        equals((totalUpdates - bufferSize).toDouble()),
      );
    });
